...
@Component({
  selector: 'app-avg-line-level',
  templateUrl: './avg-line-level.component.html',
  styleUrls: ['./avg-line-level.component.css']
})
export class AvgLineLevelComponent implements OnInit, AfterViewInit, OnChanges {
  public loading = false;
  pie_id_template:string = 'pie_avg_id_template';
  pie_strong_id_template:string = 'pie_avg_strong_id_template';
  pie_week_id_template:string = 'pie_avg_week_id_template';
  config = {title:'强弱分级'}
  ...
  changeTableData(event):void {
    ...
    stocks.forEach(item => {
      ...
      if(item.avg_line_level_week.indexOf('Up_Level')>-1){
      ...
      }else if(item.avg_line_level_week.indexOf('Down_Level')>-1){
  ...
  displayedColumns: string[] = [
    ...
    'avg_line_level_day', 
    'avg_line_level_week',  
    'avg_line_level_month',
    'end', 
    'ratio', 
    'basic_section_industry',
    'basic_section_region',
    'avg_line_level_min_15', 
    'avg_line_level_min_60'
  ];
  ...
  queryTable(): void {
    let url = environment.apiUrls.getAvgLineLevel;
    ...
    const sb = this.httpService.get<any>(url, httpParams).subscribe((data) => {
      ...
      data.forEach((item)=>{
        ...
        let stock_obj = {
          ...
          'end':item.end, 
          'ratio':item.ratio, 
          'avg_line_level_min_15':this.toAvgLineLevel(item.avg_line_level_min_15, '分15'),
          'avg_line_level_min_60':this.toAvgLineLevel(item.avg_line_level_min_60, '分60'),
          ...
          'avg_line_level_month':this.toAvgLineLevel(item.avg_line_level_month, '月'),
        ...
        if(stock_obj.avg_line_level_week.indexOf('强')>-1){
          if(trade_stocks_map_strong.has(item.basic_section_industry)){
            trade_stocks_map_strong.get(item.basic_section_industry)['value'] += 1;
          }else{
            trade_stocks_map_strong.set(item.basic_section_industry, {name:item.basic_section_industry, value: 1});
        ...
        }else if(stock_obj.avg_line_level_week.indexOf('弱')>-1){
          if(trade_stocks_map_week.has(item.basic_section_industry)){
            trade_stocks_map_week.get(item.basic_section_industry)['value'] += 1;
          }else{
            trade_stocks_map_week.set(item.basic_section_industry, {name:item.basic_section_industry, value: 1});
